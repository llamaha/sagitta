...
impl PathRelevanceScorer {
    ...
    fn tokenize(&self, input: &str) -> Vec<String> {
        ...
        for part in camel_parts {
            let snake_parts: Vec<String> = part
                .split('_')
                .filter(|s| !s.is_empty())
            ...
            for snake_part in snake_parts {
                let kebab_parts: Vec<String> = snake_part
                    .split('-')
                    .filter(|s| !s.is_empty())
                    .map(|s| s.to_lowercase())
                    .collect();
...
mod tests {
    ...
    fn test_path_parsing() {
        ...
        assert_eq!(
            parsed.dir_components,
            vec!["controllers".to_string(), "src".to_string()]
        );
    ...
    fn test_relevance_scoring() {
        ...
        assert!(
            score1 > score2,
            "Exact filename match should have higher score than non-match"
        );
        ...
        assert!(
            score3 > score4,
            "Query with user token should score higher for user.rb (score3={} vs score4={})",
            score3,
            score4
        );
        ...
        assert!(
            score5 > score6,
            "Path with controllers directory should score higher"
        );
    }
}
