...
impl VectorDB {
    pub fn new(db_path: String) -> Result<Self> {
        ...
        debug!(
            "Creating repository manager at: {}",
            repo_manager_path.display()
        );
        ...
        let repo_manager = match RepoManager::new(repo_manager_path) {
            ...
            Err(e) => {
                ...
                RepoManager::new(repo_config_path)
                    .unwrap_or_else(|_| panic!("Failed to create repository manager"))
            }
        };
            ...
            if let Some(repo) = self.repo_manager.get_repository(repo_id) {
                if let Ok(git_repo) = crate::utils::git::GitRepo::new(repo.path.clone()) {
                    if let Ok(commit_hash) = git_repo.get_commit_hash(branch) {
                        if let Err(e) =
                            self.repo_manager
                                .update_indexed_commit(repo_id, branch, &commit_hash)
                        {
    ...
    pub fn save(&mut self) -> Result<()> {
        ...
        if self.is_in_repository_context() {
            ...
            if let (Some(repo_id), Some(branch)) = (&self.current_repo_id, &self.current_branch) {
                debug!(
                    "Updating last_indexed for repository {}, branch {}",
                    repo_id, branch
                );
    ...
    pub fn clear_repository(&mut self, repo_name: &str) -> Result<()> {
        ...
        let is_current_repo = self
            .current_repo_id
            .as_ref()
            .map_or(false, |id| id == &repo_id)
            && self
                .current_branch
                .as_ref()
                .map_or(false, |b| b == &current_branch);

        if is_current_repo {
            // If we're in the context of this repository, need to clear in-memory data
            debug!(
                "Clearing in-memory data for current repository: {}",
                repo_name
            );
            ...
            debug!(
                "Repository {} is not the current active repository",
                repo_name
            );
        }
    ...
    pub fn get_similar_queries(&self, query: &str, max_queries: usize) -> Vec<String> {
        ...
        let mut scored_queries: Vec<(String, f32)> = self
            .feedback
            .query_feedback
            .keys()
            ...
            .map(|existing_query| {
                ...
                let intersection: Vec<&&str> = query_terms
                    .iter()
            ...
            if let Some(feedback_map) = self.feedback.query_feedback.get(&similar_query) {
                for (file_path, entry) in feedback_map {
                    if let Some(score) = file_scores.get_mut(file_path) {
                        ...
                        let confidence =
                            (entry.relevant_count + entry.irrelevant_count) as f32 / 10.0;
    ...
    pub fn hnsw_index(&self) -> Option<&HNSWIndex> {
        if let Some(index) = &self.hnsw_index {
            debug!(
                "HNSW index accessed: {} nodes, {} layers",
    ...
    pub fn switch_repository(&mut self, repo_id: &str, branch: Option<&str>) -> Result<()> {
        ...
        // Get the repository
        let repo = self.repo_manager.get_repository(repo_id).ok_or_else(|| {
            VectorDBError::RepositoryError(format!("Repository with ID '{}' not found", repo_id))
        })?;
        ...
        if let (Some(current_repo), Some(current_branch)) =
            (&self.current_repo_id, &self.current_branch)
        {
            debug!(
                "Saving state of current repository {}, branch {}",
            ...
            if current_repo == repo_id && current_branch != &branch_name {
                debug!(
                    "Switching branches within repository: {} -> {}",
                ...
                if let Ok(git_repo) = crate::utils::git::GitRepo::new(repo_path.clone()) {
                    ...
                    if let (Some(prev_commit), Ok(curr_commit)) = (
                        ...
                        if let Ok(common_ancestor) =
                            git_repo.find_common_ancestor(prev_commit, &curr_commit)
                        {
                            debug!(
                                "Found common ancestor between branches {} and {}: {}",
        ...
        let mut repo_updated = false;
        {
            if let Some(repo) = self.repo_manager.get_repository_mut(repo_id) {
                if repo.active_branch != branch_name {
                    debug!(
                        "Updating active branch for repository {}: {} -> {}",
    ...
    pub fn index_repository_full(&mut self, repo_id: &str, branch: &str) -> Result<()> {
        ...
        let repo_data = {
            // Get the repository
            let repo = self.repo_manager.get_repository(repo_id).ok_or_else(|| {
                VectorDBError::RepositoryError(format!(
        ...
        let commit_hash = git_repo
            .get_commit_hash(&branch)
            .map_err(|e| VectorDBError::RepositoryError(e.to_string()))?;

        // Run a full indexing on the repository
        info!(
            "Indexing repository {} ({}), branch {} at commit {}",
        ...
        if let Some(repo) = self.repo_manager.get_repository_mut(&repo_id) {
            if repo.active_branch != branch {
                info!(
                    "Setting branch {} as active branch for repository {}",
                    branch, repo_id
                );
        ...
        info!(
            "Repository {} branch {} indexed successfully",
            repo_name, branch
        );
    ...
    pub fn index_repository_changes(&mut self, repo_id: &str, branch: &str) -> Result<()> {
        debug!(
            "Incremental indexing of repository {} branch {}",
            repo_id, branch
        );
        ...
        let (repo_path, repo_name, last_commit, file_types, repo_id, branch, prev_active_branch) =
            repo_data;
        ...
        if last_commit.is_none() && branch != prev_active_branch {
            info!(
                "No previous index found for branch {}, checking for cross-branch sync from {}",
                branch, prev_active_branch
            );
            println!(
                "Attempting efficient cross-branch sync from '{}' to '{}'...",
                prev_active_branch, branch
            );

            // Check if the previous branch was indexed
            let prev_branch_commit = self
                .repo_manager
                .get_repository(&repo_id)
            ...
            if let Some(prev_commit) = prev_branch_commit {
                println!(
                    "Previous branch '{}' is indexed at commit {}...",
                    prev_active_branch,
                    &prev_commit[..8]
                );
                ...
                match git_repo.find_common_ancestor(&prev_commit, &current_commit) {
                    Ok(common_ancestor) => {
                        info!(
                            "Found common ancestor between branches: {}",
                            common_ancestor
                        );
                        ...
                        info!(
                            "Performing efficient cross-branch sync from {} to {}",
                            prev_active_branch, branch
                        );

                        let changes = git_repo
                            .get_cross_branch_changes(&common_ancestor, &current_commit)
                            .map_err(|e| VectorDBError::RepositoryError(e.to_string()))?;
                        ...
                        if total_changes == 0 {
                            ...
                            self.repo_manager.update_indexed_commit(
                                &repo_id,
                                &branch,
                                &current_commit,
                            )?;
                            ...
                            info!(
                                "Repository {} branch {} indexed successfully (no changes)",
                                repo_name, branch
                            );
                            return Ok(());
                        }
                        ...
                        let files_to_process =
                            [&changes.added_files[..], &changes.modified_files[..]].concat();

                        // Filter for file types we care about
                        let filtered_files: Vec<PathBuf> = files_to_process
                            .iter()
                            .filter(|path| {
                        ...
                        if relevant_file_count == 0 {
                            ...
                            info!(
                                "Repository {} branch {} indexed successfully (no relevant files)",
                                repo_name, branch
                            );
                            return Ok(());
                        }
                        ...
                        println!(
                            "Processing {} changed files with relevant extensions...",
                            relevant_file_count
                        );
                        ...
                        info!(
                            "Repository {} branch {} indexed successfully via cross-branch sync",
                            repo_name, branch
                        );

                        println!(
                            "Branch '{}' successfully synced from '{}' based on {} changes",
                ...
                println!(
                    "Previous branch '{}' is not indexed yet, cannot use for efficient syncing.",
                    prev_active_branch
                );
            }
            ...
            info!(
                "No previous index found for repository {} branch {}, doing full index",
                repo_name, branch
            );
        ...
        if last_commit == current_commit {
            info!(
                "Repository {} branch {} is already up to date at commit {}",
        ...
        let changes = git_repo
            .get_change_set(&last_commit, &current_commit)
            .map_err(|e| VectorDBError::RepositoryError(e.to_string()))?;

        info!(
            "Incremental indexing of repository {} branch {} from commit {} to {}",
        ...
        let total_changes =
            changes.added_files.len() + changes.modified_files.len() + changes.deleted_files.len();
        println!(
            "Found {} changes between commits {}.. and {}..:",
            total_changes,
            &last_commit[..8],
            &current_commit[..8]
        );
        ...
        info!(
            "Changes detected: {} added, {} modified, {} deleted files",
        ...
        // Filter for file types we care about
        let filtered_files: Vec<_> = files_to_process
            .iter()
            .filter(|path| {
                if let Some(ext) = path.extension() {
                    let ext_str = ext.to_string_lossy().to_string();
        ...
        // Update the indexed commit hash
        self.repo_manager
            .update_indexed_commit(&repo_id, &branch, &current_commit)?;
    ...
    fn index_directory_parallel(
        ...
        info!(
            "Starting parallel indexing for {} files using {} threads.",
        ...
        debug!("Checking cache for {} files...", file_count);
        {
            ...
            for file_path in &files {
                ...
                match cache_guard.check_cache_and_get_hash(&path_str, file_path) {
                    Ok(CacheCheckResult::Hit(embedding)) => {
                        ...
                        if let Some(index) = &mut self.hnsw_index {
                            if let Err(e) = index.insert(embedding) {
                                error!(
                                    "Failed to insert cached embedding into HNSW index for {}: {}",
                                    file_path.display(),
                                    e
                                );
                            }
                    ...
                    Err(e) => {
                        error!(
                            "Cache check/hash error for {}: {}. Queuing for embedding.",
                            file_path.display(),
                            e
                        );
                        cache_errors += 1;
        ...
        if cache_errors > 0 {
            progress.println(format!(
                "Warning: Encountered {} cache read/hash errors.",
                cache_errors
            ));
        }
        ...
        debug!(
            "Found {} files in cache. {} files need embedding.",
            files_from_cache, files_to_embed_count
        );

        if files_to_embed_count == 0 {
            progress.finish_with_message(format!(
                "Processed {} files (all from cache).",
                files_from_cache
            ));
        ...
        info!(
            "Processing {} files needing embedding in {} chunks (batch size ~{}).",
        ...
        let pool = rayon::ThreadPoolBuilder::new()
            ...
            .unwrap();
        let (tx, rx): (
            Sender<Vec<(PathBuf, Option<u64>, Result<Vec<f32>>)>>,
            Receiver<_>,
        ) = mpsc::channel();
        ...
        pool.install(move || {
            file_chunks
                .into_par_iter()
                .for_each_with(tx, |tx_clone, chunk_data| {
                    let mut texts_to_embed: Vec<String> = Vec::with_capacity(chunk_data.len());
                    let mut paths_and_hashes: Vec<(PathBuf, Option<u64>)> =
                        Vec::with_capacity(chunk_data.len());
                    let mut results_for_chunk: Vec<(PathBuf, Option<u64>, Result<Vec<f32>>)> =
                        Vec::with_capacity(chunk_data.len());
                    ...
                    for (path, hash_opt) in chunk_data {
                        match std::fs::read_to_string(&path) {
                            ...
                            Err(e) => {
                                results_for_chunk.push((
                                    path,
                                    hash_opt,
                                    Err(VectorDBError::IOError(e)),
                                ));
                            }
                        }
                    }
                    ...
                    if !texts_to_embed.is_empty() {
                        let text_refs: Vec<&str> =
                            texts_to_embed.iter().map(String::as_str).collect();
                        match model_clone.embed_batch(&text_refs) {
                            Ok(embeddings) => {
                                assert_eq!(embeddings.len(), paths_and_hashes.len());
                                for ((path, hash_opt), embedding) in
                                    paths_and_hashes.into_iter().zip(embeddings)
                                {
                            ...
                            Err(e) => {
                                ...
                                for (path, hash_opt) in paths_and_hashes {
                                    results_for_chunk.push((
                                        path,
                                        hash_opt,
                                        Err(VectorDBError::EmbeddingError(error_message.clone())),
                                    ));
                                }
                            }
                        }
        ...
        while let Ok(chunk_results) = rx.recv() {
                ...
                match result {
                    Ok(embedding) => {
                        ...
                        if let Some(hash) = hash_opt {
                            if let Err(e) = cache_guard.insert_with_hash(
                                file_path_str.clone(),
                                embedding.clone(),
                                hash,
                            ) {
                                error!(
                                    "Failed to insert into cache for {}: {}",
                                    file_path.display(),
                                    e
                                );
                            }
                            ...
                            match EmbeddingCache::get_file_hash(&file_path) {
                                Ok(new_hash) => {
                                    if let Err(e) = cache_guard.insert_with_hash(
                                        ...
                                        error!(
                                            "Failed to insert into cache (retry hash) for {}: {}",
                                            file_path.display(),
                                            e
                                        );
                                    }
                                ...
                                Err(e) => {
                                    error!(
                                        "Failed again to get hash for cache insertion for {}: {}",
                                        file_path.display(),
                                        e
                                    );
                                }
                        ...
                        // DB insert
                        self.embeddings
                            .insert(file_path_str.clone(), embedding.clone());
                        ...
                        if let Some(index) = &mut self.hnsw_index {
                            if let Err(e) = index.insert(embedding) {
                                error!(
                                    "Failed to insert into HNSW index for {}: {}",
                                    file_path.display(),
                                    e
                                );
                                progress.println(format!(
                                    "Warning: Failed to add {} to HNSW index: {}",
                                    file_path_str, e
                                ));
                            }
                        }
            ...
            if now.duration_since(last_report_time).as_secs() >= 5 {
                ...
                let rate = if elapsed_secs > 0.1 {
                    processed_new_files as f32 / elapsed_secs
                } else {
                    0.0
                };
        ...
        let rate_final = if elapsed_total > 0.1 {
            successful_embeddings as f32 / elapsed_total
        } else {
            0.0
        };
        ...
        if let (Some(repo_id), Some(branch)) = (&self.current_repo_id, &self.current_branch) {
            debug!(
                "Updating commit hash for repository {} branch {}",
                repo_id, branch
            );
            ...
            if let Some(repo) = self.repo_manager.get_repository(&repo_id_clone) {
                if let Ok(git_repo) = crate::utils::git::GitRepo::new(repo.path.clone()) {
                    if let Ok(commit_hash) = git_repo.get_commit_hash(&branch_clone) {
                        if let Err(e) = self.repo_manager.update_indexed_commit(
                            &repo_id_clone,
                            &branch_clone,
                            &commit_hash,
                        ) {
                            error!(
                                "Failed to update indexed commit for {}/{}: {}",
                                repo_id_clone, branch_clone, e
                            );
                        }
                    } else {
                        warn!(
                            "Could not get current commit hash for {}/{}",
                            repo_id_clone, branch_clone
                        );
                    }
                } else {
