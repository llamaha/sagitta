...
use crate::vectordb::repo::{canonical_repo_id_with_name, GitRepoConfig};
...
impl RepoManager {
    ...
    pub fn new(config_path: PathBuf) -> Result<Self> {
        debug!(
            "Creating repository manager with config path: {}",
            config_path.display()
        );
            ...
            match fs::read_to_string(&config_path) {
                Ok(contents) => {
                    match serde_json::from_str::<RepoManager>(&contents) {
                        Ok(mut manager) => {
                            debug!(
                                "Successfully loaded repository manager with {} repositories",
                            ...
                            for id in to_deactivate {
                                if let Some(repo) = manager.repositories.get_mut(&id) {
                                    debug!(
                                        "Repository path no longer exists, deactivating: {}",
                                        repo.path.display()
                                    );
                                    repo.active = false;
                                }
    ...
    pub fn get_active_repository(&self) -> Option<&GitRepoConfig> {
        self.active_repo_id
            .as_ref()
            .and_then(|id| self.get_repository(id))
    }
    ...
    pub fn resolve_repo_name_to_id(&self, name_or_id: &str) -> Result<String> {
        ...
        // Not found
        Err(anyhow!(
            "Repository with name or ID '{}' not found",
            name_or_id
        ))
    }

    /// Update the indexed commit hash for a repository branch
    pub fn update_indexed_commit(
        &mut self,
        repo_id: &str,
        branch: &str,
        commit_hash: &str,
        ...
        debug!(
            "Updating indexed commit for repository {}, branch {}: {}",
            repo_id, branch, commit_hash
        );

        // Get the repository
        let repo = self
            .get_repository_mut(repo_id)
    ...
    pub fn update_branch_relationship(
        &mut self,
        repo_id: &str,
        branch1: &str,
        branch2: &str,
    ...
    pub fn save(&self) -> Result<()> {
        debug!(
            "Saving repository manager configuration to {}",
