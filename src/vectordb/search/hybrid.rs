// use std::cmp::Ordering; // Removed unused
...
pub(crate) fn hybrid_search_with_limit(
    ...
    let mut vector_results_map: HashMap<String, f32> = HashMap::new();
    if v_weight > 0.0 {
        // Run vector search, limit to a larger internal number initially
        let internal_limit = (max_results as f32 * 1.5).ceil() as usize;
        let vector_results = vector::search_with_limit(
            db, model,
            query, internal_limit
        )?;
        vector_results_map = vector_results
            .into_iter()
            .map(|res| (res.file_path, res.similarity))
            .collect();
    }
    ...
    if b_weight < 1e-6 || bm25_index_opt.is_none() { // Use epsilon for float comparison
        ...
        let mut diverse_results: Vec<SearchResult> = vector_results_map.into_iter().map(|(file_path, score)| SearchResult {
            file_path,
            similarity: score,
        }).collect();
    ...
    let mut normalized_vector_results = vector_results_map.into_iter().map(|(file_path, score)| SearchResult {
        file_path,
        similarity: score,
    }).collect();
