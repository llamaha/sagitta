...
impl GitRepoConfig {
    ...
    pub fn update_indexed_commit<S: AsRef<str>>(&mut self, branch: &str, commit_hash: S) {
        self.indexed_branches
            .insert(branch.to_string(), commit_hash.as_ref().to_string());
        ...
        if !self.branch_relationships.contains_key(branch) {
            self.branch_relationships
                .insert(branch.to_string(), BranchRelationship::new());
        }
    }
    ...
    pub fn update_branch_relationship(
        ...
        if !self.branch_relationships.contains_key(branch1) {
            self.branch_relationships
                .insert(branch1.to_string(), BranchRelationship::new());
        }
        if !self.branch_relationships.contains_key(branch2) {
            self.branch_relationships
                .insert(branch2.to_string(), BranchRelationship::new());
        }
...
mod tests {
    ...
    fn test_canonical_repo_id_with_name() -> Result<()> {
        ...
        let name = "test-repo";
        let config =
            GitRepoConfig::new(repo_path.clone(), Some(name.to_string()), name.to_string())?;
