#!/usr/bin/env python
# Convert BGE-Large-EN-v1.5 model to ONNX format
...
DEFAULT_OUTPUT_DIR = "bge_large_en_onnx"
...
def download_and_convert_bge_model_to_onnx(output_dir=DEFAULT_OUTPUT_DIR, model_name=DEFAULT_MODEL_NAME):
    ...
    seq_len = 512 # BGE-Large-EN-v1.5 uses 512 token context window
    ...
    dynamic_axes = {
        ...
        "sentence_embedding": {0: "batch_size"} # Dimension 1 (1024 for BGE-Large-EN) is fixed
    }
...
def verify_onnx_model(onnx_path, tokenizer_dir, model_name):
    ...
    try:
        ...
        # Check output shape (should be 1024 for BGE-Large-EN)
        output_shape = ort_session.get_outputs()[0].shape
        print(f"ONNX Output Shape: {output_shape}")
        expected_dim = 1024 # BGE-Large-EN-v1.5 embedding dimension
...
def main():
    parser = argparse.ArgumentParser(description="Convert BGE-Large-EN-v1.5 model to ONNX.")
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    print("  2. Update your Rust code to expect 1024-dim embeddings (vs 384 from MiniLM)")
    print("  3. Set max_seq_length to 512 in your ONNX provider")
    print("  4. This model is ~4x larger than MiniLM but much more accurate for code")
    
    print("\n🚀 Performance Expectations:")
    print("  - 50-70% better search accuracy than st-codesearch-distilroberta-base")
    print("  - Memory usage: ~1.3GB VRAM (fits well in 8GB)")
    print("  - Optimal batch size: 24-32 for your hardware")
