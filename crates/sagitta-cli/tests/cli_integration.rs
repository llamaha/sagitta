...
use sagitta_search::config::{AppConfig, RepositoryConfig, save_config, load_config, get_config_path_or_default};
use sagitta_cli::cli::commands::{FIELD_FILE_PATH, FIELD_BRANCH, FIELD_COMMIT_HASH};
...
async fn test_cli_index_list_remove() -> Result<()> {
    ...
    let count = get_qdrant_point_count(&client, "sagitta-code-search", None, None, None).await?;
...
fn test_cli_clear_failures() -> Result<()> {
    ...
    let _ = fs::remove_dir_all(config_dir.join("sagitta-cli"));
    ...
    Command::new(&bin_path)
        .env("SAGITTA_ONNX_MODEL", model_path.to_str().unwrap()) // Ensure ONNX paths are set
        .env("SAGITTA_ONNX_TOKENIZER_DIR", tokenizer_dir.to_str().unwrap())
        .arg("repo")
        .arg("clear")
        .arg("-y")
        .assert()
    ...
    Command::new(&bin_path)
        ...
        .arg("--name")
        .arg("dummy-for-clear")
        .env("SAGITTA_ONNX_MODEL", model_path.to_str().unwrap())
        .env("SAGITTA_ONNX_TOKENIZER_DIR", tokenizer_dir.to_str().unwrap())
        .assert()
        .success();
...
fn test_index_rejects_conflicting_onnx_args() -> Result<()> {
    ...
    std::env::set_var("XDG_CONFIG_HOME", config_dir.to_str().unwrap());
    std::env::set_var("XDG_DATA_HOME", data_dir.to_str().unwrap());
    let _ = fs::remove_dir_all(config_dir.join("sagitta-cli")); // Ensure no prior config
    ...
    Command::new(&bin_path)
        .env("SAGITTA_ONNX_MODEL", dummy_model_path.to_str().unwrap())
        .env("SAGITTA_ONNX_TOKENIZER_DIR", dummy_tokenizer_dir.to_str().unwrap())
        .arg("simple")
        .arg("index")
...
fn test_index_requires_config_onnx_args() -> Result<()> {
    ...
    // Create dummy ONNX files that simple clear might need access to for config init
    let dummy_model_path = temp_dir.path().join("dummy.onnx");
    let dummy_tokenizer_dir = temp_dir.path().join("dummy_tokenizer");
    fs::create_dir_all(&dummy_tokenizer_dir)?;
    fs::write(&dummy_model_path, "dummy_onnx_content_for_clear_test")?;
    fs::write(dummy_tokenizer_dir.join("tokenizer.json"), "dummy_tokenizer_content_for_clear")?;

    // --- Clear collection before test --- 
    println!("Clearing simple index collection before test_index_requires_config_onnx_args...");
    Command::new(&bin_path)
        .env("QDRANT_URL", "http://localhost:6334") 
        .env("SAGITTA_ONNX_MODEL", dummy_model_path.to_str().unwrap()) // Add ONNX env var
        .env("SAGITTA_ONNX_TOKENIZER_DIR", dummy_tokenizer_dir.to_str().unwrap()) // Add ONNX env var
        .arg("simple")
        .arg("clear")
        .assert()
        .success(); // Should succeed even if collection doesn't exist or is already clear
    std::thread::sleep(std::time::Duration::from_millis(500)); // Give Qdrant a moment

    println!("Testing simple index rejection with ONNX paths in env only (expect config required error)...");

    // Re-declare these for clarity for the main part of the test, though paths are the same
    let target_dir_model_path = temp_dir.path().join("dummy.onnx");
    let target_dir_tokenizer_dir = temp_dir.path().join("dummy_tokenizer");
    let target_dir = temp_dir.path().join("target_dir");
    fs::create_dir_all(&target_dir)?;
    fs::write(target_dir.join("file.txt"), "content")?;

    // Set ONNX paths ONLY via environment variables for the command being tested
    Command::new(&bin_path)
        .env("SAGITTA_ONNX_MODEL", target_dir_model_path.to_str().unwrap())
        .env("SAGITTA_ONNX_TOKENIZER_DIR", target_dir_tokenizer_dir.to_str().unwrap())
        .arg("simple")
        .arg("index")
        .arg(target_dir.to_str().unwrap())
        .assert()
...
async fn test_repo_sync_scenarios() -> Result<()> {
    ...
    let config_dir = temp_dir.path().join("config");
    let config_path = config_dir.join("sagitta-cli/config.toml");

    fs::create_dir_all(config_dir.join("sagitta-cli"))?;
    ...
    Command::new(&bin_path)
        .env("SAGITTA_ONNX_MODEL", model_path.to_str().unwrap()) // Needed if collection needs creation
        .env("SAGITTA_ONNX_TOKENIZER_DIR", tokenizer_dir.to_str().unwrap())
        .env("QDRANT_URL", &qdrant_url)
        .arg("repo")
        .arg("add")
    ...
    Command::new(&bin_path)
        .env("SAGITTA_ONNX_MODEL", model_path.to_str().unwrap()) // Model needed for re-indexing
        .env("SAGITTA_ONNX_TOKENIZER_DIR", tokenizer_dir.to_str().unwrap())
        .env("QDRANT_URL", &qdrant_url)
        .arg("repo")
        .arg("sync")
