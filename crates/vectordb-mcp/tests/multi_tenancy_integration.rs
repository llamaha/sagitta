//! Integration test for multi-tenancy, API key, and OAuth flows in vectordb-mcp.
...
"#;
    ...
    let mut cmd = Command::new("../../target/release/vectordb-mcp");
    cmd.args(["http", "--host", "127.0.0.1", "--port", "8082"])
        .stdout(Stdio::inherit())
        .stderr(Stdio::inherit());
    cmd.env("VECTORDB_TEST_CONFIG_PATH", temp_config.path().to_str().unwrap());
    cmd.env("VECTORDB_BOOTSTRAP_ADMIN_KEY", "test_admin_key");
