...
pub async fn handle_tools_call<C: QdrantClientTrait + Send + Sync + 'static>(
    ...
    let result: Result<CallToolResult, ErrorObject> = match tool_name {
        ...
        //     }
        // }
        // DISABLED: Reducing scope for stability
...
".to_string()
        ...
        ToolDefinition {
            ...
            description: Some("Executes shell commands. Must include ONE of: tail_lines (see end of output), head_lines (see start of output), or grep_pattern (filter output). Examples: 'cargo build' with tail_lines=50, 'cargo test' with grep_pattern='error'. Working directory defaults to current repository.".to_string()),
            input_schema: json!({
                "type": "object",
                "properties": {
...
mod tests {
    ...
    // DISABLED: These tests are for tools we disabled
    // #[test]
