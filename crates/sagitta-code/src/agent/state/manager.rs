...
impl StateManager {
    ...
    pub async fn set_agent_state(&self, new_state: AgentState, reason: impl Into<String>) -> Result<(), FredAgentError> {
    ...
    pub async fn set_idle(&self, reason: impl Into<String>) -> Result<(), FredAgentError> {
        self.set_agent_state(AgentState::Idle, reason).await
    }
    
    /// Set the agent to thinking state
    pub async fn set_thinking(&self, reason: impl Into<String>) -> Result<(), FredAgentError> {
        let reason_str = reason.into();
        self.set_agent_state(AgentState::Thinking { message: reason_str.clone() }, reason_str).await
    }
    
    /// Set the agent to responding state
    pub async fn set_responding(&self, streaming: bool, reason: impl Into<String>) -> Result<(), FredAgentError> {
    ...
    pub async fn set_executing_tool(&self, tool_call_id: impl Into<String>, tool_name: impl Into<String>, reason: impl Into<String>) -> Result<(), FredAgentError> {
    ...
    pub async fn set_error(&self, message: impl Into<String>, reason: impl Into<String>) -> Result<(), FredAgentError> {
        self.set_agent_state(
            AgentState::Error {
    ...
    pub async fn set_agent_mode(&self, mode: AgentMode) -> Result<(), FredAgentError> {
        let mut state = self.state.write().await;
    ...
    pub async fn set_conversation_status(&self, status: ConversationStatus) -> Result<(), FredAgentError> {
        let mut state = self.state.write().await;
    ...
    pub async fn set_conversation_failed(&self, error: impl Into<String>) -> Result<(), FredAgentError> {
        self.set_conversation_status(ConversationStatus::Paused).await
    }
    
    /// Set whether typing indicators are enabled
    pub async fn set_typing_indicator(&self, enabled: bool) -> Result<(), FredAgentError> {
        let mut state = self.state.write().await;
    ...
    pub async fn clear_transitions(&self) -> Result<(), FredAgentError> {
        let mut state = self.state.write().await;
